...
| `P1.h_min` | m | 推荐工作点 (T*, h0=1500) 下**自由飞行段（开伞前）轨迹最低海拔**，由飞行动力学决定（≈h0+600~700 m），是直接计算出的定值，**不是 h0 扫描下限** | Q1 第二问 |
...
2. **results.json 存全精度浮点，不四舍五入**（显示精度由 fill.py 在统稿时定，SPEC 不卡精度）。近零临界值（|值| < 0.01）用科学计数法保留有效位（如 `3.99e-05`），避免归零丢失「紧约束」信息；论文里相应注明「该约束为紧约束（临界）」。
